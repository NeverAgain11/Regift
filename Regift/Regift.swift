...
public struct Regift {
    ...
    public func createGIFForTimePoints(_ timePoints: [TimePoint], fileProperties: [String: AnyObject], frameProperties: [String: AnyObject], frameCount: Int, size: CGSize? = nil) throws -> URL {
        ...
        let oneTimePerImages = 200
        
        var timeArray = [[NSValue]]()
        var arrayIndex = -1
        for time in timePoints {
            if timeArray.last == nil || timeArray.last?.count == oneTimePerImages {
                timeArray.append([NSValue]())
                arrayIndex += 1
            }
            timeArray[arrayIndex].append(NSValue(time: time))
        }
